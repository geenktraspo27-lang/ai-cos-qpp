-- 課題4: when complete_workflow() creates a Completion Report, also save the
-- outcome to Brain as Knowledge — reusing the existing public.ideas table
-- (Brain Room's data source) rather than introducing a new table/Room.
-- Fixed-template content only; no LLM summarization in this step.

alter table public.ideas
  add column content text,
  add column source_document_id uuid references public.documents (id) on delete set null;

-- One Knowledge entry per Completion Report, ever.
create unique index ideas_source_document_id_key
  on public.ideas (company_id, source_document_id)
